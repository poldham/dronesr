...
When the collection has been created we are in a position to download all of the records. Look for the `Export` button on the results page. You will then see the pop up box in Figure \@ref(fig:export). Choose 50,000 records and optionally give the file a name. You will want to export in .csv format (the default).
...
We can obtain the Patents Cited by our core dataset by choosing the `Cites Patents` tab in the results. For ease of reference we will use the export button and save then as cited patents as in Figure \@ref(fig:citedpat).
...
We turn to the lens scholarly interface to search the literature as in Figure \@ref(fig:litsearch). As with the patent data we will search all fields. But take the time to check out the options that can be used here. 
